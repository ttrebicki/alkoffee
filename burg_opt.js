...
// wzór klatki (dystans pokonany na osi x / dystans pokonany na osi y -> dopasować do odp. przypadków):
  ...
  var anim;
// współrzędne x2 i y2 po zakończeniu animacji powracającej / współrzędne x2 i y2 początkowe (te, które są ustawione w pliku HTML):
  if (xTwo === x1.toString() && yTwo === y1.toString() || lineNum.hasAttribute("x2") == false || lineNum.hasAttribute("y2") == false) {
...
// współrzędne x2 i y2 po zakończeniu animacji:
  else if (xTwo === x2.toString() && yTwo === y2.toString()) {
...
function leftDownX(lineNum, x2, y2, x1, y1) {
...
function leftDown(lineNum, x2, y2, x1, y1) {
  if (x1 < y2) {
    leftDownY(lineNum, x2, y2, x1, y1);
  }
  else {
    leftDownX(lineNum, x2, y2, x1, y1);
  }
}

var lineArr = [leftDown(lineOne, 0, 50, 25, 0), leftDown(lineTwo, 0, 10, 40, 0)];
